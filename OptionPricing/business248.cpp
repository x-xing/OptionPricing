...
namespace QuantLib {
    ...
    std::string Business248::Impl::name() const {
        std::ostringstream out;
        out << "Business/248(" << calendar_.name() << ")";
        return out.str();
    }

    Date::serial_type Business248::Impl::dayCount(const Date& d1,
                                                  const Date& d2) const {
    ...
    Time Business248::Impl::yearFraction(const Date& d1,
                                         const Date& d2,
                                         const Date&,
                                         const Date&) const {
        return dayCount(d1, d2)/248.0;
    }
